...
is
   STANDARD_VARIABLE;
   lLower epsilon.lower%type;
   lLower_unit epsilon.lower_unit%type;
   lUpper epsilon.upper%type;
   lUpper_unit epsilon.upper_unit%type;
   lDiff int := 0;
   lRetValue int := 0;
   lExpCount int; -- used instead of lExpectedCount due to assignment.
begin
   lExpCount := lExpectedCount;
   if lExpCount = 0 and lActualCount = 0 then
      return 0;
   end if;
      
   select lower,lower_unit,upper,upper_unit
      into lLower,lLower_unit,lUpper,lUpper_unit
      from sde_it.epsilon
