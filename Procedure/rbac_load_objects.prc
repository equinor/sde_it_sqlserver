...
------------------------------------------------
-- Include this.Picks up feature class/dataset.
-- Uses som dirty tricks
-- Or simplify existing code
------------------------------------------------
/*
with xpos as (select charindex('.',name) as pos,name from sde.gdb_items gi ) 
,xgbd_tab as (select substring(gi.name,1,(xp.pos -1)) as xschema
   ,substring(gi.name,(xp.pos +1),200) as object_name
   ,upper(gi2.name) as object_type
   ,null as object_flags
   ,null as xcomment
   --,gi.name
   --,xp.pos
...
inner join xpos xp
  on xp.name=gi.Name 
  and xp.pos > 0   
 inner join sde.gdb_itemtypes gi2 
    on gi.[Type] = gi2.UUID 
    and lower(gi2.name) in  ('feature class','feature dataset')
)
,xtable_reg as (select owner as xschema
	,case when tr.imv_view_name is not null then 
		tr.imv_view_name  
	  else
	    table_name  
	  end as object_name
	...
	,object_flags
	,case when tr.imv_view_name is not null then 
	    'IMV_VEIW. Base_table: '+ tr.owner + '.' + tr.table_name + '.'
	  end as xcomment
	/*,case when tr.imv_view_name is not null then 1 else 0 end as is_imv_view*/
--	  ,tr.object_flags 
  from sde.sde_table_registry tr
  where (tr.object_flags in (15,16391,114703) -- Feature class (FC)
          or tr.imv_view_name is not null -- dirty trick to get version FC
  --      or tr.object_flags in (16399,98315,114703,278535) --Versioned FC
        )
)
,xdata as (select xschema,object_name,object_type,xcomment
      from xgbd_tab 
   union
   select xschema,object_name,object_type,xcomment 
      from xtable_reg
)
select *
  from xdata
  where xschema not in ('SDE');
*/
